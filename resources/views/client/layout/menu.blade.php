                ...
                <li class="menu_brands dropdown"><a href="#">برند ها</a>
                    <div class="dropdown-menu">
                        @foreach($brands as $brand)
                            {{--  str_replace('public', '/storage', $brand->image) => '$brand->image' جایگزین کن در داخل رشته  '/storage' رو دیدی را با 'public' بگرد هر جا که کلمه   --}}
                            <div class="col-lg-1 col-md-2 col-sm-3 col-xs-6"><a href="#"><img src="{{str_replace('public', '/storage', $brand->image)}}" title="{{$brand->name}}" alt="{{$brand->name}}" width="50px" height=""/></a><a href="#">{{$brand->name}}</a></div>
                        @endforeach
                    </div>
                </li>
